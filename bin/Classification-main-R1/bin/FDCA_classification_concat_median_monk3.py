...
class FDCA:
    ...
    def godal_encoding(orginal_data):
        #**********Godal number encoding ex Datset=2,1,1 equalent to 2^2+3^1+5^1=60
        N = max(len(x) for x in orginal_data)
        prime_list = FDCA.generatePrime(N)
        #print(prime_list)
        #print("data for encoding", orginal_data[0])
        orginal_data_godal = []
        for obj1 in orginal_data:
            godal_enc = 1
            i = 0
            for ele in obj1:
                godal_enc = godal_enc*pow(prime_list[i], int(ele))
                i = i+1
            res = [int(x) for x in str(godal_enc)]
            orginal_data_godal.append(res)
        maxLength = max(len(x) for x in orginal_data_godal)
        print(maxLength)
        orginal_data_godal_padded = []
        for ele in orginal_data_godal:
            ele = ([0] * maxLength + ele)[-maxLength:]
            orginal_data_godal_padded.append(ele)
        #print("modified", orginal_data_godal_padded)
        return (orginal_data_godal_padded)
    ...
    def fit(self):
        ...
        data_minority=self.data[self.data.Class==0]
        data_minority_upsampled=resample(data_minority,
                                        replace=True,
                                        n_samples=228,
                                        random_state=123)
        ...
        # train_data_configuration = copy.deepcopy(FDCA.godal_encoding(train_data_configuration))

        # test_data_configuration = copy.deepcopy(FDCA.godal_encoding(test_data_configuration))
...
def CA_classification(self, CA, train_data_configuration, train_data_class,
    ...
    for ca in range(0,len(CA)):
        ...
        if self.compare:
            my_data = [[self.best_so_far, Heir_sill_new,
                        Kmeans_sill_new, Birch_new]]
            head = ["FDCA", "ML1", "ML2", "ML3"]
            print(tabulate(my_data, headers=head, tablefmt="grid"))

        #print("---------------------***-----------------******----------------------***--------------------")


RCA = FDCA('monks-3-new',
