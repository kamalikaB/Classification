...
class Preprocessor:
    ...
    def preprocess_training_data(self, X_train, y_train):
        ...
        X_train['Pregnancies'] = (X_train['Pregnancies'] * 1).astype(int)
        X_train['Pregnancies'] = X_train['Pregnancies'].astype(str).apply(lambda x: x.zfill(2))
        

      
        X_train['Glucose'] = (X_train['Glucose'] * 1).astype(int)
        X_train['Glucose'] = X_train['Glucose'].astype(str).apply(lambda x: x.zfill(3))


  
        X_train['BloodPressure'] = (X_train['BloodPressure'] * 1).astype(int)
        X_train['BloodPressure'] = X_train['BloodPressure'].astype(str).apply(lambda x: x.zfill(2))
        
        
  
        X_train['SkinThickness'] = (X_train['SkinThickness'] * 1).astype(int)
        X_train['SkinThickness'] = X_train['SkinThickness'].astype(str).apply(lambda x: x.zfill(2))
        

        
        X_train['Insulin'] = (X_train['Insulin'] * 1).astype(int)
        X_train['Insulin'] = X_train['Insulin'].astype(str).apply(lambda x: x.zfill(3))
 

      
        X_train['BMI'] = (X_train['BMI'] * 10).astype(int)
        X_train['BMI'] = X_train['BMI'].astype(str).apply(lambda x: x.zfill(3))

        
        X_train['DiabetesPedigreeFunction'] = (X_train['DiabetesPedigreeFunction'] * 1000).astype(int)
        X_train['DiabetesPedigreeFunction'] = X_train['DiabetesPedigreeFunction'].astype(str).apply(lambda x: x.zfill(3))
        

        
       

        X_train['Age'] = (X_train['Age'] * 1).astype(int)
        X_train['Age'] = X_train['Age'].astype(str).apply(lambda x: x.zfill(2))
    ...
    def preprocess_test_data(self, X_test, y_test):
        ...
        X_test['Pregnancies'] = (X_test['Pregnancies'] * 1).astype(int)
        X_test['Pregnancies'] = X_test['Pregnancies'].astype(str).apply(lambda x: x.zfill(2))
        

      
        X_test['Glucose'] = (X_test['Glucose'] * 1).astype(int)
        X_test['Glucose'] = X_test['Glucose'].astype(str).apply(lambda x: x.zfill(3))


  
        X_test['BloodPressure'] = (X_test['BloodPressure'] * 1).astype(int)
        X_test['BloodPressure'] = X_test['BloodPressure'].astype(str).apply(lambda x: x.zfill(2))
        
        
  
        X_test['SkinThickness'] = (X_test['SkinThickness'] * 1).astype(int)
        X_test['SkinThickness'] = X_test['SkinThickness'].astype(str).apply(lambda x: x.zfill(2))
        

        
        X_test['Insulin'] = (X_test['Insulin'] * 1).astype(int)
        X_test['Insulin'] = X_test['Insulin'].astype(str).apply(lambda x: x.zfill(3))
 

      
        X_test['BMI'] = (X_test['BMI'] * 10).astype(int)
        X_test['BMI'] = X_test['BMI'].astype(str).apply(lambda x: x.zfill(3))

        
        X_test['DiabetesPedigreeFunction'] = (X_test['DiabetesPedigreeFunction'] * 1000).astype(int)
        X_test['DiabetesPedigreeFunction'] = X_test['DiabetesPedigreeFunction'].astype(str).apply(lambda x: x.zfill(3))
        

        
       

        X_test['Age'] = (X_test['Age'] * 1).astype(int)
        X_test['Age'] = X_test['Age'].astype(str).apply(lambda x: x.zfill(2))
        # Process numerical columns
...
for ca in range(0,len(CA)):
  ...
  with open("./results/diabetes.txt", "a") as file:
