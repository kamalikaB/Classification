...
class Preprocessor:
    ...
    def preprocess_training_data(self, X_train, y_train):
        ...
        #X_train['A1'] = (X_train['A1'] * 10).astype(int)
        X_train['Age'] = X_train['Age'].astype(str).apply(lambda x: x.zfill(2))

        #X_train['A2'] = (X_train['A2'] * 10).astype(int)
        X_train['year'] = X_train['year'].astype(str).apply(lambda x: x.zfill(2))

        #X_train['A3'] = (X_train['A3'] * 1).astype(int)
        X_train['axillary nodes detected'] = X_train['axillary nodes detected'].astype(str).apply(lambda x: x.zfill(2))
    ...
    def preprocess_test_data(self, X_test, y_test):
        ...
        X_test['year'] = X_test['year'].astype(str).apply(lambda x: x.zfill(2))

        #X_test['A3'] = (X_test['A3'] * 1).astype(int)
        X_test['axillary nodes detected'] = X_test['axillary nodes detected'].astype(str).apply(lambda x: x.zfill(2))
        # Process numerical columns
...
for ca in range(0,len(CA)):
  ...
  with open("./results/haber.txt", "a") as file:
